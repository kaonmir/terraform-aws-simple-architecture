locals {
  image_url  = "%{if var.ecr.registry != ""}${var.ecr.registry}/%{endif}${var.ecr.repository}:${var.ecr.tag}"
...
data "template_file" "user_data" {
  template = <<-EOT
    ...
    sudo aws ecr get-login-password --region ${var.aws_region} | docker login --username AWS --password-stdin ${var.ecr.registry}
